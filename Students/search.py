...
def SearchAction(keywords: list[str], index: int = 0):
    title = bold('[5] Tìm kiếm thông tin lớp')
    cls(title)
    DATA = Read()
    data = DATA.copy()
    dt = [0, 0, 0, 1, 0, 0, 0]
    output1 = '    '+ '     '.join(
        list(map(lambda i: option(str(i[0]+1), i[1][0]), FIELDS.items()))
    )
    output3 = '    ' + '    '.join([
            ...
            try:
                ...
                while True:
                    ...
                    if c == '1':
                        data = r
                        break
                    elif c == '2': return ['s-u', [d[0] for d in r[1:]], ['s-s', keywords, index]]
                    elif c == '3': return ['s-r', [d[0] for d in r[1:]], ['s-s', keywords, index]]
